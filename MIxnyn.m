...
function miout=MIxnyn(x,y,uniqueId,kneig);
...
zwsp=[x;y]';
fname = strcat('/scratch/cluster/aditya/mutualInfo/logs/lstm/misc_logs/zwspMIxnyn', uniqueId, '.txt')  
save (fname, 'zwsp', '-ASCII')
cmd = ['MIxnyn ', fname, ' ']

% execute C Programm
[a unout]=unix([cmd ,num2str(Ndx),' ',num2str(Ndy),' ',num2str(N),' ',num2str(kneig)]);
miout=str2num(unout);
unix(['rm -f ', fname]);
